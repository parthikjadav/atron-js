...
describe("getJSON", () => {
  ...
  test(
    ...
    withMockFetch(
      ...
      async () => {
        const urls = [400, 401, 403, 404, 500, 502, 503].map(
          (code) => `https://example.com/status/${code}`,
        );

        for (const url of urls) {
          // Updated regex to match new error message: "Request to ... failed with status ..."
          await assert.rejects(getJSON(url), /failed with status/);
        }
      },
    ),
  );
    ...
    withMockFetch(
      ...
      async () => {
        ...
        for (const url of urls) {
          ...
          await assert.rejects(getJSON(url), /Expected JSON from/);
        }
      },
    ),
  );
  ...
  test(
    ...
    withMockFetch(
      ...
      async () => {
        await assert.rejects(getJSON("https://example.com/slow", { timeoutMs: 10 }), /timed out/);
      },
    ),
  );
